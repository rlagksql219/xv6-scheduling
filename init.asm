...
putc(int fd, char c)
{
 ...
 488:	e8 c5 fe ff ff       	call   352 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 5c8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5cb:	b8 20 08 00 00       	mov    $0x820,%eax
...
free(void *ap)
{
 ...
 651:	a1 cc 0a 00 00       	mov    0xacc,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 6a7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 6a9:	a3 cc 0a 00 00       	mov    %eax,0xacc
}
 6ae:	5b                   	pop    %ebx
 6af:	5e                   	pop    %esi
 6b0:	5f                   	pop    %edi
 ...
 6ca:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 6cd:	a3 cc 0a 00 00       	mov    %eax,0xacc
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 6e9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ec:	8b 15 cc 0a 00 00    	mov    0xacc,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
