...
int main(void)
{
   ...
   e:	83 ec 04             	sub    $0x4,%esp
	get_num_proc();
  11:	e8 fc 02 00 00       	call   312 <get_num_proc>
	exit();
...
putc(int fd, char c)
{
 ...
 3b8:	e8 c5 fe ff ff       	call   282 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 428:	50                   	push   %eax
 429:	57                   	push   %edi
 42a:	e8 53 fe ff ff       	call   282 <write>
...
putc(int fd, char c)
{
 ...
 48a:	50                   	push   %eax
 48b:	57                   	push   %edi
 48c:	e8 f1 fd ff ff       	call   282 <write>
...
putc(int fd, char c)
{
 ...
 56f:	50                   	push   %eax
 570:	57                   	push   %edi
 571:	e8 0c fd ff ff       	call   282 <write>
...
free(void *ap)
{
 ...
 581:	a1 a0 09 00 00       	mov    0x9a0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5d7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 5d9:	a3 a0 09 00 00       	mov    %eax,0x9a0
}
 5de:	5b                   	pop    %ebx
 5df:	5e                   	pop    %esi
 5e0:	5f                   	pop    %edi
 ...
 5fa:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 5fd:	a3 a0 09 00 00       	mov    %eax,0x9a0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 619:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 61c:	8b 15 a0 09 00 00    	mov    0x9a0,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
