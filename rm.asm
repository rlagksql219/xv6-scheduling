...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3df:	f7 f6                	div    %esi
 3e1:	0f b6 92 94 07 00 00 	movzbl 0x794(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 418:	e8 c5 fe ff ff       	call   2e2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 488:	50                   	push   %eax
 489:	57                   	push   %edi
 48a:	e8 53 fe ff ff       	call   2e2 <write>
...
putc(int fd, char c)
{
 ...
 4ea:	50                   	push   %eax
 4eb:	57                   	push   %edi
 4ec:	e8 f1 fd ff ff       	call   2e2 <write>
...
putc(int fd, char c)
{
 ...
 558:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 55b:	b8 8d 07 00 00       	mov    $0x78d,%eax
...
free(void *ap)
{
 ...
 637:	89 08                	mov    %ecx,(%eax)
  freep = p;
 639:	a3 38 0a 00 00       	mov    %eax,0xa38
}
 63e:	5b                   	pop    %ebx
 63f:	5e                   	pop    %esi
 640:	5f                   	pop    %edi
 ...
 65a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 65d:	a3 38 0a 00 00       	mov    %eax,0xa38
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 71c:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 71f:	89 15 38 0a 00 00    	mov    %edx,0xa38
      return (void*)(p + 1);
 ...
 734:	eb e9                	jmp    71f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 736:	c7 05 38 0a 00 00 3c 	movl   $0xa3c,0xa38
 73d:	0a 00 00 
 740:	c7 05 3c 0a 00 00 3c 	movl   $0xa3c,0xa3c
 747:	0a 00 00 
    base.s.size = 0;
 74a:	b8 3c 0a 00 00       	mov    $0xa3c,%eax
 74f:	c7 05 40 0a 00 00 00 	movl   $0x0,0xa40
 756:	00 00 00 
